"""Retrain v3 (AstarNetV3, 192 hidden, 8 dilated ResBlocks) on 90 GT samples.
...
log = logging.getLogger('v3_retrain')
...
# ── v3 Architecture (exact match to nn_predict.py AstarNetV3) ────────
...
class AstarNetV3(nn.Module):
    """V3: 192 hidden, 8 dilated ResBlocks, multi-scale."""
...
def main():
    log.info("=" * 60)
    log.info("RETRAIN V3 (AstarNetV3) on R1-R18 (90 samples)")
    log.info("=" * 60)

    samples, round_z = load_dataset()
    log.info(f"Loaded {len(samples)} samples")

    # Full training only — no LORO (NF LORO is the gate, v3 is ensemble filler)
    best_state, best_score = train_model(samples, epochs=1500, tag="FULL")
    log.info(f"Best full-data score: {best_score:.1f}")
    torch.save(best_state, "astar_nn_v3_r18.pt")
    log.info("Saved astar_nn_v3_r18.pt")
    log.info("Done!")
